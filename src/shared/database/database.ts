...
import { Demographics, ApiKey } from '../types/demographics';
...
class DatabaseService {
  ...
  constructor() {
    // config object instead of connectionString
    this.config = {
  ...
  // Demographics operations - Fixed to match your Zod schema
  async createDemographic(demographic: Demographics): Promise<void> {
    const pool = await this.getPool();
    const request = pool.request();

    // Complete SQL query with ALL fields from your Demographics schema
    const query = `
      INSERT INTO Demographics (
        id, partitionKey, law_firm, law_firm_approval, firstname, lastname, email, phone,
        sf_id, ml_id, law_firm_client_id, otherid, primarylawfirm, ethnicity, claimanttype,
        liensfinal, bankruptcy, bankruptcycleared, probate, probatecleared, pathway_opt_in_status,
    ...
    // Add all parameters - Fixed to match your actual Demographics schema fields
  ...
  async getDemographicById(id: string, lawFirm: string): Promise<Demographics | null> {
    ...
    // Return the row directly - no need to parse JSON since fields are flat
  ...
  async getDemographicsByLawFirm(
    ...
    // Return rows directly - all fields are flat in your schema
    return result.recordset as Demographics[];
  }

  // API Key operations (fixed to match your ApiKey schema)
  async createApiKey(apiKey: ApiKey): Promise<void> {
    const pool = await this.getPool();
    const request = pool.request();

    await request
      .input('id', sql.UniqueIdentifier, apiKey.id)
      .input('partitionKey', sql.VarChar(75), apiKey.partitionKey)
      .input('key_id', sql.VarChar(50), apiKey.key_id)
      .input('key_hash', sql.VarChar(255), apiKey.key_hash)
      .input('name', sql.VarChar(100), apiKey.name)
      .input('description', sql.VarChar(500), apiKey.description)
      .input('law_firm', sql.VarChar(60), apiKey.law_firm) // Fixed: Match your schema max length
      .input('created_by', sql.UniqueIdentifier, apiKey.created_by)
      .input('rate_limits', sql.NVarChar(sql.MAX), JSON.stringify(apiKey.rate_limits))
      .input('scopes', sql.NVarChar(sql.MAX), JSON.stringify(apiKey.scopes))
      .input('status', sql.VarChar(20), apiKey.status)
      .input('usage_count', sql.Int, apiKey.usage_count)
      .input('expires_at', sql.DateTime2, apiKey.expires_at ? new Date(apiKey.expires_at) : null)
      .input('created_at', sql.DateTime2, new Date(apiKey.created_at))
      .input('updated_at', sql.DateTime2, new Date(apiKey.updated_at))
      .input('allowed_ips', sql.NVarChar(sql.MAX), apiKey.allowed_ips ? JSON.stringify(apiKey.allowed_ips) : null)
      .input('allowed_domains', sql.NVarChar(sql.MAX), apiKey.allowed_domains ? JSON.stringify(apiKey.allowed_domains) : null)
      .input('environment', sql.VarChar(20), apiKey.environment)
      .query(`
        INSERT INTO ApiKeys (
          id, partitionKey, key_id, key_hash, name, description, law_firm,
          created_by, rate_limits, scopes, status, usage_count, expires_at,
